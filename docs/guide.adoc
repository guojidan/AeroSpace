...
==== Workspace overview and up/down switch

`workspace` supports `next|prev|up|down` targets for relative workspace switching.
`next` and `down` move forward in the workspace list.
`prev` and `up` move backward.

You can use xref:commands.adoc#workspace-overview[workspace-overview]
to quickly inspect workspace names and switching order, then use
xref:commands.adoc#workspace[workspace] `up`/`down` bindings to navigate:

[source,toml]
----
[mode.main.binding]
    alt-slash = 'workspace-overview'
    alt-j = 'workspace down'
    alt-k = 'workspace up'
----

Migration note for the default config:

* `alt-slash` previously toggled `layout`; it is now `workspace-overview`
* `alt-j` / `alt-k` previously focused windows down/up; they are now `workspace down` / `workspace up`
* Layout toggles moved to `alt-backslash` / `alt-period`
* Window focus down/up moved to `alt-ctrl-j` / `alt-ctrl-k`
...
In total, AeroSpace provides 6 possible layouts:
...
- `v_scroll` vertical scroll (`scroll` preserves current orientation)
...
Scroll peek is configurable via `scroll-peek-padding` option (default: `32`).
Scroll main-pane ratio is configurable via `scroll-main-pane-ratio` option (default: `0.9`).
Scroll resize step is configurable via `scroll-main-pane-ratio-step` option (default: `0.05`).

Scroll layout keeps one focused "main pane" and partially reveals immediate neighbors.
The focused pane ratio is preserved while focus moves between windows.
